...
let getArtifacts = function(directory) {
    ...
    if (!_.isString(artifactsDirectory)) {
        logger.error('getArtifacts', 'artifactsDirectory', 'missing');
        return;
    }
...
let getConfiguration = function(artifacts, branch, token, notes) {
    ...
    if (artifactList.length === 0) {
        logger.error('createConfiguration', 'artifactList', 'empty');
        return;
    }

    if (!_.isString(releaseBranch)) {
        logger.error('createConfiguration', 'releaseBranch', 'missing');
        return;
    }

    if (!_.isString(githubToken)) {
        logger.error('createConfiguration', 'githubToken', 'missing');
        return;
    }

    if (!_.isString(releaseNotes)) {
        logger.error('createConfiguration', 'releaseNotes', 'missing');
        return;
    }

    if (!_.isObjectLike(packageJson)) {
        logger.error('createConfiguration', 'package', 'missing');
        return;
    }
    ...
    logger.debug('createConfiguration', util.inspect(releaseConfiguration));
