...
let parseLogEntry = function() {
    ...
    return {
        chalk: chalkStyleMap[type],
        message: message,
        namespace: _.toLower(namespace),
        rgb: rgbColorMap[type],
        title: title,
        type: _.toUpper(type),
    };
};
...
let printBrowserMessage = function() {
    ...
    writeToFile(util.format(`[${parameters.type}] [%s] [%s] %s`, parameters.namespace, parameters.title, parameters.message));
};
...
let printMessage = function() {
    ...
    log(util.format('[%s] %s %s', parameters.chalk(parameters.namespace), parameters.chalk.bold(parameters.title), parameters.chalk(parameters.message)));
    writeToFile(util.format(`[${parameters.type}] [%s] [%s] %s`, parameters.namespace, parameters.title, parameters.message));
};
