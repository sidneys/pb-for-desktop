...
let build = () => {
    ...
    childProcess.execSync(buildCommand, { cwd: appRootPath, stdio: [0, 1, 2] });
};

/** @namespace fs.copySync */
/** @namespace fs.execSync */
/** @namespace fs.removeSync */
...
let main = () => {
    ...
    if (platformHelper.isMacOS) {
        let sourceFilePath = path.normalize(path.join(installerDirectory, 'mac', `${appProductName}.app`));
        ...
        fs.copySync(sourceFilePath, destinationFilePath, { overwrite: true });
    ...
    if (platformHelper.isWindows) {
        let installerFilePathPattern = path.join(installerDirectory, 'win', `*${appProductName}*${appVersion}.exe`);
        let installerFilePathList = globby.sync(installerFilePathPattern);
    ...
    if (platformHelper.isLinux) {
        ...
        let installerFilePathPattern = path.normalize(path.join(installerDirectory, `*${appName}*${appVersion}*${installerArch}*.deb`));
        let installerFilePathList = globby.sync(installerFilePathPattern);
        let installerFilePath = installerFilePathList[0];
